import { PrismaClient, UserType, VerificationLevel, BookingStatus, PaymentStatus, BookingSource, PaymentMethod } from '@prisma/client';
...
async function main() {
  ...
  const fullSession = await prisma.serviceType.findUnique({ where: { name: 'حصة تعليمية كاملة' } });
  const quickHelp = await prisma.serviceType.findUnique({ where: { name: 'شرح مسألة سريعة' } });
  ...
  // 4. Parents & Students
  const parentsData = [
    { name: 'أبو أحمد', email: 'parent1@test.com', phone: '0599111111', students: [{ name: 'أحمد', grade: 10 }, { name: 'سارة', grade: 6 }] },
    { name: 'أم محمد', email: 'parent2@test.com', phone: '0599222222', students: [{ name: 'محمد', grade: 12 }] },
    { name: 'أبو يوسف', email: 'parent3@test.com', phone: '0599333333', students: [{ name: 'يوسف', grade: 5 }, { name: 'ليان', grade: 8 }, { name: 'عمر', grade: 9 }] },
  ];
  ...
  for (const p of parentsData) {
    ...
    if (!parent) {
      ...
      parent = await prisma.user.findUnique({ where: { email: p.email }, include: { students: true }});
    }
  ...
  // 5. Teachers
  const teachersData = [
    { name: 'أستاذ خالد', email: 'khaled@test.com', spec: 'رياضيات', subSpec: 'التوجيهي العلمي', city: 'رام الله', levels: [10, 11, 12], rate: 60, level: VerificationLevel.GOLD, bio: 'مدرس رياضيات بخبرة 15 عاماً في تدريس التوجيهي.' },
    { name: 'معلمة منى', email: 'mona@test.com', spec: 'رياضيات', subSpec: 'المرحلة الأساسية', city: 'نابلس', levels: [5, 6, 7, 8, 9], rate: 45, level: VerificationLevel.SILVER, bio: 'مختصة في تأسيس الطلاب في مادة الرياضيات.' },
    { name: 'أستاذ سامي', email: 'sami@test.com', spec: 'فيزياء', subSpec: 'فيزياء توجيهي', city: 'الخليل', levels: [11, 12], rate: 70, level: VerificationLevel.GOLD, bio: 'أقوم بتبسيط الفيزياء المعقدة بطرق مبتكرة.' },
    { name: 'معلمة ليلى', email: 'layla@test.com', spec: 'لغة إنجليزية', subSpec: 'محادثة وتأسيس', city: 'رام الله', levels: [1, 2, 3, 4, 5, 6, 7, 8], rate: 40, level: VerificationLevel.BRONZE, bio: 'مدرسة لغة إنجليزية وأسعى لكسر حاجز الخوف من التحدث.' },
    { name: 'أستاذ عمر', email: 'omar@test.com', spec: 'لغة عربية', subSpec: 'نحو وصرف', city: 'جنين', levels: [8, 9, 10, 11, 12], rate: 50, level: VerificationLevel.SILVER, bio: 'عاشق للغة الضاد، أعلم النحو بأسلوب عصري.' },
  ];
  ...
  for (const t of teachersData) {
    ...
    if (!tUser) {
      ...
      const ts1 = await prisma.teacherService.create({
        data: { teacherId: teacher.id, serviceTypeId: fullSession.id, price: t.rate, duration: 60 }
      });
      teacherServices.push(ts1);
      
      if (Math.random() > 0.5) {
        await prisma.teacherService.create({
           data: { teacherId: teacher.id, serviceTypeId: quickHelp.id, price: t.rate * 0.4, duration: 15 }
        });
      }
      ...
      for (let day = 1; day <= 4; day++) {
        await prisma.teacherAvailability.create({
          data: { teacherId: teacher.id, dayOfWeek: day, startTime: '16:00', endTime: '20:00' }
        });
      }
    } else {
      const teacher = await prisma.teacher.findUnique({ where: { userId: tUser.id }});
      if(teacher) {
        teacherObjects.push(teacher);
        const tss = await prisma.teacherService.findMany({ where: { teacherId: teacher.id }});
  ...
  const bookingScenarios = [
    ...
    { status: BookingStatus.CANCELLED, paymentStatus: PaymentStatus.REFUNDED, daysOffset: -2, withReport: false, withReview: false },
  ];

  let bookingCounter = 0;
  for (let i = 0; i < 15; i++) {
    const scenario = bookingScenarios[i % bookingScenarios.length];
    const teacherSvc = teacherServices[Math.floor(Math.random() * teacherServices.length)];
    const parent = parentUsers[Math.floor(Math.random() * parentUsers.length)];
    const student = parent.students[Math.floor(Math.random() * parent.students.length)];
    ...
    const booking = await prisma.booking.create({
      data: {
        ...
        meetingUrl: [BookingStatus.CONFIRMED, BookingStatus.COMPLETED].includes(scenario.status as any) ? 'https://meet.jit.si/edunest-test-meeting' : null,
        confirmedAt: [BookingStatus.CONFIRMED, BookingStatus.COMPLETED].includes(scenario.status as any) ? new Date() : null,
        completedAt: scenario.status === BookingStatus.COMPLETED ? new Date() : null,
        cancelledAt: scenario.status === BookingStatus.CANCELLED ? new Date() : null,
        cancellationReason: scenario.status === BookingStatus.CANCELLED ? 'ظرف طارئ' : null,
      }
    });
  ...
  console.log(`✅ تم إنشاء ${bookingCounter} حجز بحالات مختلفة (مع التقارير، التقييمات والمدفوعات)`);
  ...
  for (const p of parentsData) console.log(`🔹 ولي أمر (${p.name}): ${p.email}`);
