      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10" dir="rtl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 relative">
          ...
          {steps.map((step, idx) => (
            ...
            >
              {/* Step Number Bubble */}
              <div className={`absolute -top-6 left-1/2 -translate-x-1/2 w-12 h-12 rounded-2xl bg-gradient-to-br ${step.gradient} flex items-center justify-center text-xl font-black text-white shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                {idx + 1}
              </div>

              {/* Icon */}
              <div className={`mx-auto w-16 h-16 rounded-full bg-gradient-to-br ${step.gradient} flex items-center justify-center shadow-inner mb-6 mt-4`}>
                {step.icon}
              </div>
