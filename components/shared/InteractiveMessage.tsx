...
export type MessageMood = 'neutral' | 'happy' | 'encouraging' | 'warning' | 'advice';
    ...
    <div className={cn('flex items-start gap-4 animate-fade-in-up', className)}>
      ...
      <div className={cn(
        ...
        <div className="relative z-10 space-y-2">
          <h4 className={cn("text-sm font-black flex items-center gap-1.5", titleStyles)} suppressHydrationWarning>
            {title || (isHakeem ? 'الحكيم يقول:' : 'نجيب يقول:')}
          </h4>
          <div className="text-sm leading-relaxed font-semibold text-slate-700 dark:text-slate-300" suppressHydrationWarning>
            {message}
          </div>
        </div>
      </div>
