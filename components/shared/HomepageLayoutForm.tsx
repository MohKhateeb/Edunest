...
export default function HomepageLayoutForm({
	...
	const [newSectionType, setNewSectionType] =
		useState<SectionType>("custom_html");
	...
	const addNewSection = () => {
		...
		switch (newSectionType) {
			case "custom_html":
				defaultProps = {
					title: "قسم مخصص جديد",
					html: '<div class="py-12 text-center bg-gray-50 border rounded-2xl"> محتوى مخصص </div>',
				};
				break;
			case "hero":
				defaultProps = {
					headline: "عنوان البانر الرئيسي",
					subheadline: "وصف قصير للبانر",
					primaryBtnText: "رابط رئيسي",
					primaryBtnLink: "#",
					secondaryBtnText: "رابط فرعي",
					secondaryBtnLink: "#",
				};
				break;
			case "stats":
				defaultProps = {
					items: [{ label: "عنصر إحصائي", value: 100, suffix: "+" }],
				};
				break;
			case "subjects":
				defaultProps = {
					title: "المواد الدراسية",
					subjects: ["رياضيات", "عربي"],
				};
				break;
			case "features":
				defaultProps = {
					title: "ميزاتنا",
					subtitle: "وصف المميزات",
					items: [
						{
			...
			case "how_it_works":
				defaultProps = {
					title: "طريقة العمل",
					subtitle: "شرح بسيط",
					items: [{ num: "١", title: "خطوة أولى", desc: "تفاصيل الخطوة" }],
				};
				break;
			...
			case "testimonials":
				defaultProps = {
					title: "تجارب مستخدمين",
					subtitle: "ماذا قالوا عنا",
					items: [{ text: "محتوى التقييم", author: "الاسم", city: "المدينة" }],
				};
				break;
			case "faq":
				defaultProps = {
					title: "الأسئلة الشائعة",
					subtitle: "استفسارات شائعة",
				};
				break;
			case "cta":
				defaultProps = {
					title: "انضم إلينا الآن",
					subtitle: "سجل فوراً بالمنصة",
					primaryBtnText: "سجل كطالب",
					primaryBtnLink: "/register",
					secondaryBtnText: "سجل كمعلم",
	...
	const renderSectionForm = (section: Section) => {
		...
		switch (section.type) {
			...
			case "stats":
				return <StatsSectionForm {...props} />;
			case "subjects":
				return <SubjectsSectionForm {...props} />;
			case "features":
				return <FeaturesSectionForm {...props} />;
			case "how_it_works":
				return <HowItWorksSectionForm {...props} />;
			case "featured_teachers":
				return <FeaturedTeachersSectionForm {...props} />;
			case "testimonials":
				return <TestimonialsSectionForm {...props} />;
			case "faq":
				return <FaqSectionForm {...props} />;
			case "cta":
				return <CtaSectionForm {...props} />;
			case "custom_html":
				return <CustomHtmlSectionForm {...props} />;
			default:
				return <div>نموذج غير معروف.</div>;
		}
	};
				...
				<div className="border-t border-border pt-4 space-y-3">
					...
					<div className="flex gap-2">
						...
						<button
							...
							className="bg-primary/10 text-primary p-2.5 rounded-lg"
						>
				...
				<div className="border-t border-border pt-4">
					...
					>
						{loading ? (
							...
							<>
								<Save className="w-4 h-4" /> حفظ التخطيط
							</>
						)}
					</button>
				</div>
			...
			<div className="lg:col-span-2 space-y-6">
				...
				{activeTab ? (
					(() => {
						...
						if (!section)
							return (
								<div className="bg-card border rounded-xl p-8 text-center text-xs">
									القسم غير موجود.
								</div>
							);
						const labelInfo = SECTION_TYPE_LABELS[section.type];
						...
						return (
							<div className="bg-card border border-border rounded-xl p-6 space-y-6 shadow-sm animate-fade-in">
								<div className="border-b border-border pb-4 flex flex-col md:flex-row md:items-center justify-between gap-2">
									<div>
										<h2 className="font-extrabold text-base text-foreground flex items-center gap-2">
											<span>
												تعديل محتوى: {labelInfo?.label || section.type}
											</span>
								...
								<div className="space-y-5">{renderSectionForm(section)}</div>
							</div>
						);
					})()
				) : (
