...
import { sendDisputeMessage, resolveDispute, changeDisputeTurn } from '@/lib/actions/disputes';
import { UserType, DisputeStatus, DisputeTurn } from '@prisma/client';
import { Shield, GraduationCap, User, Info } from 'lucide-react';
...
export function DisputeChat({ disputeId, status, allowedTurn, messages, currentUserId, currentUserType }: DisputeChatProps) {
  ...
  const canSend = currentUserType === 'ADMIN' || allowedTurn === 'BOTH' || 
  ...
  const getTurnMessage = () => {
    ...
    if (allowedTurn === 'PARENT' && currentUserType === 'TEACHER') return 'الإدارة بانتظار رد ولي الأمر حالياً. لا يمكنك الإرسال.';
    if (allowedTurn === 'TEACHER' && currentUserType === 'PARENT') return 'الإدارة بانتظار رد المعلم حالياً. لا يمكنك الإرسال.';
    return '';
  };
            ...
            <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} ${isAdminMsg ? 'items-center' : ''}`}>
              {!isAdminMsg && (
                <div className="flex items-center gap-1.5 text-xs text-gray-500 mb-1 mx-1">
                  {roleIcon}
                  <span>{msg.sender.name} ({roleLabel})</span>
                </div>
              )}
            ...
            <div className="mt-4 space-y-4">
              <div className="p-4 border border-blue-200 bg-blue-50 dark:bg-blue-900/10 rounded-xl">
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  <button type="button" onClick={() => handleTurnChange('BOTH')} disabled={loading} className={`flex-1 min-w-[120px] py-1.5 rounded-lg text-xs font-bold border transition-all ${allowedTurn === 'BOTH' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-blue-600 border-blue-200 hover:bg-blue-50 dark:bg-gray-800 dark:border-gray-700'}`}>الجميع</button>
                  <button type="button" onClick={() => handleTurnChange('PARENT')} disabled={loading} className={`flex-1 min-w-[120px] py-1.5 rounded-lg text-xs font-bold border transition-all ${allowedTurn === 'PARENT' ? 'bg-amber-600 text-white border-amber-600' : 'bg-white text-amber-600 border-amber-200 hover:bg-amber-50 dark:bg-gray-800 dark:border-gray-700'}`}>انتظار ولي الأمر</button>
                  <button type="button" onClick={() => handleTurnChange('TEACHER')} disabled={loading} className={`flex-1 min-w-[120px] py-1.5 rounded-lg text-xs font-bold border transition-all ${allowedTurn === 'TEACHER' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-emerald-600 border-emerald-200 hover:bg-emerald-50 dark:bg-gray-800 dark:border-gray-700'}`}>انتظار المعلم</button>
                  <button type="button" onClick={() => handleTurnChange('NONE')} disabled={loading} className={`flex-1 min-w-[120px] py-1.5 rounded-lg text-xs font-bold border transition-all ${allowedTurn === 'NONE' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-red-600 border-red-200 hover:bg-red-50 dark:bg-gray-800 dark:border-gray-700'}`}>قفل على الجميع</button>
                </div>
              </div>
