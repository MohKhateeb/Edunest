...
import { Section, AVAILABLE_ICONS } from '@/types/homepage-layout';
import { Plus, Trash2 } from 'lucide-react';

interface FormProps {
  section: Section;
  updateProp: (id: string, prop: string, value: any) => void;
  updateItemInProp?: (id: string, prop: string, index: number, field: string, value: any) => void;
...
export function StatsSectionForm({ section, updateItemInProp, addItemInProp, deleteItemInProp }: FormProps) {
  return (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <label className="text-[11px] font-bold text-muted-foreground block">المواد الدراسية (مفصولة بفاصلة)</label>
        <textarea
          rows={3}
          value={section.props.subjects?.join('، ') || ''}
          onChange={(e) => {
            const values = e.target.value.split(/[،,]\s*/).filter((v: string) => v.trim() !== '');
...
export function FeaturesSectionForm({ section, updateProp, updateItemInProp, addItemInProp, deleteItemInProp }: FormProps) {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1.5">
      ...
      <div className="border-t border-border pt-4 space-y-4">
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={() => addItemInProp?.(section.id, 'items', { iconName: 'ShieldCheck', title: 'ميزة جديدة', desc: 'الوصف هنا.' })}
            className="bg-primary/10 text-primary hover:bg-primary/20 text-[10px] px-3 py-1.5 rounded-lg flex items-center gap-1 font-bold transition-all"
          >
...
export function HowItWorksSectionForm({ section, updateProp, updateItemInProp, addItemInProp, deleteItemInProp }: FormProps) {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1.5">
      ...
      <div className="border-t border-border pt-4 space-y-4">
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={() => addItemInProp?.(section.id, 'items', { num: '١', title: 'خطوة جديدة', desc: 'الوصف هنا.' })}
            className="bg-primary/10 text-primary hover:bg-primary/20 text-[10px] px-3 py-1.5 rounded-lg flex items-center gap-1 font-bold transition-all"
          >
...
export function FeaturedTeachersSectionForm({ section, updateProp }: FormProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div className="space-y-1.5">
        <label className="text-[11px] font-bold text-muted-foreground block">الحد الأقصى للعدد</label>
        <input
          type="number"
      ...
      <div className="space-y-1.5 md:col-span-3">
        <label className="text-[11px] font-bold text-muted-foreground block">الوصف (اختياري)</label>
        <input
          type="text"
          value={section.props.subtitle || ''}
          onChange={(e) => updateProp(section.id, 'subtitle', e.target.value)}
...
export function TestimonialsSectionForm({ section, updateProp, updateItemInProp, addItemInProp, deleteItemInProp }: FormProps) {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          <label className="text-[11px] font-bold text-muted-foreground block">الوصف أو العنوان الفرعي</label>
          <input
            type="text"
            value={section.props.subtitle || ''}
            onChange={(e) => updateProp(section.id, 'subtitle', e.target.value)}
      ...
      <div className="border-t border-border pt-4 space-y-4">
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={() => addItemInProp?.(section.id, 'items', { text: 'تجربة ممتازة جداً.', author: 'مستخدم جديد', city: 'المدينة' })}
            className="bg-primary/10 text-primary hover:bg-primary/20 text-[10px] px-3 py-1.5 rounded-lg flex items-center gap-1 font-bold transition-all"
          >
          ...
          {(section.props.items || []).map((item: any, idx: number) => (
            <div key={idx} className="flex flex-col gap-3 p-3 bg-muted/20 border border-border rounded-xl relative">
              <div className="space-y-1 w-full">
                <label className="text-[10px] text-muted-foreground block font-bold">نص الرأي / التقييم</label>
                <textarea
                  rows={2}
                  value={item.text || ''}
              ...
              <div className="flex flex-col md:flex-row gap-3">
                <div className="flex-1 space-y-1">
                  <label className="text-[10px] text-muted-foreground block font-bold">اسم كاتب الرأي</label>
                  <input
                    type="text"
                    value={item.author || ''}
                ...
                <div className="flex-1 space-y-1">
                  <label className="text-[10px] text-muted-foreground block font-bold">المدينة أو الوصف</label>
                  <input
                    type="text"
                    value={item.city || ''}
