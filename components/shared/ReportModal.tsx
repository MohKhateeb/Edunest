...
export default function ReportModal({ bookingId, onClose }: ReportModalProps) {
  ...
  return (
    <Portal>
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/75 overflow-y-auto animate-in fade-in duration-200" dir="rtl">
        <div 
        ...
        >
          ...
          <form onSubmit={handleReportSubmit} className="bg-card rounded-xl p-6 space-y-5">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 bg-accent/30 p-3.5 rounded-xl border border-border/50">
                ...
                />
                <label htmlFor="attended" className="font-bold cursor-pointer text-foreground text-sm">هل حضر الطالب الجلسة بشكل فعلي؟</label>
              </div>

              {reportForm.studentAttended ? (
                <>
                  <div className="space-y-1.5">
                    <label className="block text-xs font-bold text-muted-foreground">أداء الطالب وتقييمه (1-5)</label>
                    <select
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-xs font-bold text-muted-foreground">المواضيع التي تم تغطيتها ومناقشتها *</label>
                    <textarea
                      required
                      rows={3}
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-xs font-bold text-muted-foreground">الواجبات المنزلية المقررة (اختياري)</label>
                    <textarea
                      rows={2}
                ...
                <div className="p-4 bg-amber-50 dark:bg-amber-950/30 rounded-xl border border-amber-200 dark:border-amber-900/50">
                  <p className="text-xs font-bold text-amber-800 dark:text-amber-400">
                    بما أن الطالب لم يحضر الجلسة، لست بحاجة لكتابة تقرير أكاديمي. فقط اترك ملاحظة إن أردت (اختياري) وسنحتفظ بحقك المادي لهذه الجلسة.
                  </p>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-muted-foreground">
                  {reportForm.studentAttended ? "ملاحظات المعلم لولي الأمر (اختياري)" : "ملاحظات حول الغياب (اختياري)"}
                </label>
                <textarea
                  rows={2}
                  value={reportForm.teacherNotes}
                  onChange={(e) => setReportForm({ ...reportForm, teacherNotes: e.target.value })}
                  placeholder={reportForm.studentAttended ? "ملاحظات أو توصيات إضافية لولي الأمر لمساعدة الطالب..." : "سبب الغياب (إذا كنت تعلم) أو أي تفاصيل أخرى..."}
                  className="w-full premium-input text-sm resize-none bg-background/50 border-border"
                />
              </div>
            </div>
