...
import { 
  ...
  Percent, Clock, Banknote, ShieldAlert, BadgeCheck, Settings2, HandCoins, Info, Zap, Briefcase
} from 'lucide-react';
...
const SETTINGS_DICT: Record<string, SettingConfig> = {
  'DefaultCommissionRate': { label: 'نسبة العمولة الافتراضية', type: 'percentage', icon: Percent, category: 'FINANCIAL' },
  'QuickHelpCommissionRate': { label: 'عمولة طلبات الفزعة السريعة', type: 'percentage', icon: Zap, category: 'FINANCIAL' },
  'MonthlyPackageCommissionRate': { label: 'عمولة الحقيبة الشهرية', type: 'percentage', icon: Briefcase, category: 'FINANCIAL' },
  'MinBookingPrice': { label: 'الحد الأدنى لسعر الجلسة', type: 'currency', icon: Banknote, category: 'FINANCIAL' },
  'FreeTrialEnabled': { label: 'تفعيل الجلسات المجانية', type: 'boolean', icon: BadgeCheck, category: 'TRIAL' },
  'FreeTrialDurationMinutes': { label: 'مدة الجلسة المجانية (دقائق)', type: 'number', icon: Clock, category: 'TRIAL' },
  'FreeTrialCostToPlatform': { label: 'تكلفة الجلسة المجانية (على المنصة)', type: 'currency', icon: HandCoins, category: 'TRIAL' },
  'MaxRefundRequests': { label: 'الحد الأقصى لطلبات الاسترداد', type: 'number', icon: ShieldAlert, category: 'POLICY' },
  'CancellationRefundHours': { label: 'فترة الإلغاء المجاني (ساعات)', type: 'number', icon: Clock, category: 'POLICY' },
  'MinBookingLeadHours': { label: 'الحد الأدنى للوقت قبل الحجز (ساعات)', type: 'number', icon: Clock, category: 'POLICY' },
};

const CATEGORIES = {
  FINANCIAL: { label: 'الإدارة المالية والعمولات', icon: Banknote, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/20' },
  TRIAL: { label: 'إعدادات الجلسات المجانية', icon: BadgeCheck, color: 'text-indigo-600 dark:text-indigo-400', bg: 'bg-indigo-50 dark:bg-indigo-950/20' },
  POLICY: { label: 'سياسات الحجز والإلغاء', icon: ShieldAlert, color: 'text-rose-600 dark:text-rose-400', bg: 'bg-rose-50 dark:bg-rose-950/20' },
  OTHER: { label: 'إعدادات أخرى', icon: Settings2, color: 'text-slate-600 dark:text-slate-400', bg: 'bg-slate-50 dark:bg-slate-800' },
};
...
export default function AdminSettingsForm({ initialSettings }: AdminSettingsFormProps) {
  ...
  const groupedSettings = settings.reduce((acc, current) => {
    const config = SETTINGS_DICT[current.settingKey] || { label: current.settingKey, type: 'text', icon: Settings2, category: 'OTHER' };
    ...
    <div className="space-y-8" dir="rtl">
      <div>
        <h1 className="text-3xl font-black text-slate-800 dark:text-slate-100 flex items-center gap-3 mb-2">
          إعدادات النظام الديناميكية <Settings2 className="w-8 h-8 text-primary" />
        </h1>
        <p className="text-slate-500">تحكم بالعمولات، سياسات الإلغاء، الجلسات المجانية والقواعد الأساسية للمنصة بكل سهولة.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        {(Object.keys(CATEGORIES) as Array<keyof typeof CATEGORIES>).map((catKey) => {
          ...
          return (
            <div key={catKey} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              {/* Category Header */}
              <div className={cn("px-6 py-4 flex items-center gap-3 border-b border-slate-100 dark:border-slate-800", categoryDef.bg)}>
                <div className={cn("p-2 rounded-xl bg-white dark:bg-slate-800 shadow-sm", categoryDef.color)}>
                  <CategoryIcon className="w-5 h-5" />
                </div>
                <h2 className={cn("text-lg font-bold", categoryDef.color)}>{categoryDef.label}</h2>
              </div>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {catItems.map(({ setting, config }) => {
                  ...
                  return (
                    <div key={setting.id} className="space-y-3 relative group">
                        ...
                        <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input 
                              ...
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleToggleChange(setting.settingKey, e.target.checked)}
                            />
                          ...
                          <span className={cn("text-sm font-bold", setting.settingValue === 'true' ? 'text-emerald-600' : 'text-slate-400')}>
                            {setting.settingValue === 'true' ? 'مفعل نشط' : 'معطل'}
                          </span>
                        </div>
                      ) : (
                        <div className="relative">
                          <input
                            type={config.type === 'number' || config.type === 'percentage' || config.type === 'currency' ? 'number' : 'text'}
                            required
                            step="any"
                          ...
                          {config.type === 'currency' && (
                            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xs">شيكل</div>
                          )}
                        </div>
                      )}

                      {(setting.description || config.category === 'OTHER') && (
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 flex items-start gap-1.5 leading-relaxed">
                          ...
                          {setting.description || `Key: ${setting.settingKey}`}
                        </p>
                      )}
                    </div>
                  );
