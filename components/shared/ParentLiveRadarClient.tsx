...
export default function ParentLiveRadarClient({
	students,
	serviceTypes,
	subjects,
		...
		return (
			<div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-8 animate-in fade-in zoom-in duration-500">
				<div className="relative">
					...
					<div
						className="absolute inset-0 bg-indigo-500/10 rounded-full animate-ping scale-110"
	...
	return (
		<div className="max-w-3xl mx-auto">
			<div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-[2rem] p-8 text-white mb-8 shadow-xl shadow-indigo-500/20 relative overflow-hidden">
				<div className="absolute top-0 start-0 w-64 h-64 bg-white/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3"></div>
			...
			<form
				...
				className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6 md:p-10 shadow-sm space-y-8"
			>
						...
						{serviceTypes.map((st) => (
							...
							>
								...
								<div className="font-black text-indigo-600 dark:text-indigo-400">
									{formatCurrency(st.fazaaPrice, st.fazaaPriceCurrency)}
								</div>
							</label>
						))}
					</div>
				...
				>
					...
					{t('abhth_an_malm_alan')}({formatCurrency(selectedServiceType?.fazaaPrice ?? 0, selectedServiceType?.fazaaPriceCurrency)})
				</button>
			</form>
		</div>
	);
