  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-card border border-border p-3 rounded-lg shadow-lg">
        <p className="font-semibold text-xs mb-1 text-muted-foreground">{label}</p>
    ...
    <div className="space-y-6 mt-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-card rounded-2xl p-6 lg:col-span-2">
          <div className="mb-4">
            <h3 className="font-extrabold text-lg">أرباح المنصة (آخر 14 يوم)</h3>
            <p className="text-xs text-muted-foreground">مجموع العمولات المحصلة من الحجوزات المكتملة</p>
          </div>
        ...
        <div className="glass-card rounded-2xl p-6">
          <div className="mb-4">
            ...
            <p className="text-xs text-muted-foreground">نظرة عامة على سير العمليات</p>
          </div>
          <div className="h-[300px] w-full flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={bookingStatuses} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={5} dataKey="value" stroke="none">
                  {bookingStatuses.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={STATUS_COLORS[entry.name] || '#94a3b8'} />
                  ))}
                </Pie>
                <Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid hsl(var(--border))', backgroundColor: 'hsl(var(--card))', color: 'hsl(var(--foreground))' }} itemStyle={{ fontWeight: 'bold' }} />
                <Legend verticalAlign="bottom" height={36} iconType="circle" formatter={(value) => <span className="text-xs font-semibold text-foreground/80 ms-2">{value}</span>} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card rounded-2xl p-6">
          <div className="mb-4">
            ...
            <p className="text-xs text-muted-foreground">عدد الحجوزات الفعلية حسب تخصص المعلم</p>
          </div>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={requestedSpecializations} layout="vertical" margin={{ top: 10, right: 30, left: 20, bottom: 0 }}>
                ...
                <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))', fontWeight: 600 }} dx={-10} />
                <Tooltip cursor={{ fill: 'hsl(var(--accent))', opacity: 0.4 }} content={<CustomTooltip />} />
                <Bar dataKey="count" name="الحجوزات" fill="hsl(var(--primary))" radius={[0, 6, 6, 0]} barSize={25}>
        ...
        <div className="glass-card rounded-2xl p-6">
          <div className="mb-4">
            ...
            <p className="text-xs text-muted-foreground">توزيع الحجوزات بناءً على نوع الخدمة</p>
          </div>
          <div className="h-[300px] w-full flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={sessionTypes} cx="50%" cy="50%" innerRadius={0} outerRadius={100} paddingAngle={2} dataKey="count" stroke="none" labelLine={false} label={({ cx = 0, cy = 0, midAngle = 0, innerRadius = 0, outerRadius = 0, percent = 0, index = 0 }: any) => {
      ...
      <div className="glass-card rounded-2xl p-6">
        <div className="mb-4">
          ...
          <p className="text-xs text-muted-foreground">توزيع قاعدة الطلاب بناءً على مراحلهم الدراسية</p>
        </div>
        <div className="h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={registeredGrades} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border))" opacity={0.4} />
              <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))', fontWeight: 600 }} dy={10} />
              <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} allowDecimals={false} />
              <Tooltip cursor={{ fill: 'hsl(var(--accent))', opacity: 0.4 }} content={<CustomTooltip />} />
              <Bar dataKey="count" name="عدد الطلاب" fill="#0ea5e9" radius={[6, 6, 0, 0]} barSize={40} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
