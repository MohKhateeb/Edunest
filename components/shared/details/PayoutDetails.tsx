  ...
  return (
    <div className="space-y-6 text-xs text-muted-foreground">
      ...
      <div className="p-5 bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-2xl flex justify-between items-center flex-wrap gap-4">
        <div>
          <span className="text-[10px] block font-mono">رقم مستند التسوية: #{payout.id.toUpperCase()}</span>
          <h3 className="text-base font-extrabold text-foreground mt-0.5">
            مستحقات المعلم: أ. {payout.teacher.user.name}
          </h3>
          <span className="text-[10px] text-muted-foreground block mt-1">
            الفترة المشمولة: {new Date(payout.periodStart).toLocaleDateString('ar-EG')} إلى {new Date(payout.periodEnd).toLocaleDateString('ar-EG')}
          </span>
        </div>
        <div className="text-right">
          <span className="text-[10px] text-muted-foreground block font-bold">الحالة المالية</span>
          {payout.isPaid ? (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-4 border border-border bg-card rounded-xl text-center">
          <span className="text-[10px] text-muted-foreground block font-bold">إجمالي رسوم الحصص</span>
          <span className="text-sm font-extrabold text-foreground">{formatPrice(Number(payout.totalAmount))}</span>
        </div>
        <div className="p-4 border border-border bg-card rounded-xl text-center text-rose-600 dark:text-rose-400">
          <span className="text-[10px] text-muted-foreground block font-bold">خصم عمولة المنصة</span>
          <span className="text-sm font-extrabold">-{formatPrice(Number(payout.commissionAmount))}</span>
        </div>
        <div className="p-4 border border-border bg-card rounded-xl text-center text-purple-600 dark:text-purple-400">
          <span className="text-[10px] text-muted-foreground block font-bold">تعويض الحصص المجانية</span>
          <span className="text-sm font-extrabold">+{formatPrice(Number(payout.trialCompensation))}</span>
        </div>
        <div className="p-4 border border-border bg-emerald-500/10 rounded-xl text-center text-primary font-bold">
          <span className="text-[10px] text-primary block font-bold">الصافي المحول للمعلم</span>
          <span className="text-base font-extrabold">{formatPrice(Number(payout.netAmount))}</span>
        </div>
      </div>
          ...
          <table className="w-full text-right border-collapse text-xs">
            ...
            <tbody>
              {payout.bookings.map((booking: any) => {
                ...
                const netAmount = isFree ? Number(booking.trialCostToPlatform) : (price - commission);

                return (
                  <tr key={booking.id} className="border-b border-border last:border-none hover:bg-accent/20">
                    <td className="p-3">
                      <span className="font-bold text-foreground/80 block">{booking.student.name}</span>
                      <span className="text-[10px] text-muted-foreground">{booking.teacherService.serviceType.name}</span>
                    </td>
                    <td className="p-3 text-muted-foreground">
                      {new Date(booking.startTime).toLocaleDateString('ar-EG')} - {new Date(booking.startTime).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}
                    </td>
                    <td className="p-3">
                      {isFree ? (
                        <span className="text-[10px] font-bold text-purple-600 bg-purple-100 px-1.5 py-0.5 rounded">
                          مجانية (تعويض: {formatPrice(Number(booking.trialCostToPlatform))})
                        </span>
                      ) : (
                    ...
                    <td className="p-3 text-rose-600 dark:text-rose-400">
                      {isFree ? '-' : `-${formatPrice(commission)} (${commRate}%)`}
                    </td>
