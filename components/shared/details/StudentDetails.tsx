...
import { GraduationCap, School, Star, User, Clock, Calendar, FileText } from 'lucide-react';
...
export default function StudentDetails({ student, activeTab, setActiveTab }: StudentDetailsProps) {
  ...
  const avgPerformance = completedReports.length > 0
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 bg-gradient-to-br from-primary/10 via-primary/5 to-accent/30 border border-primary/15 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-extrabold text-foreground">{student.name}</h3>
      ...
      {activeTab === 'info' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-5 border border-border bg-accent/10 rounded-xl space-y-3">
            ...
            <div className="text-xs space-y-2">
              <div className="flex justify-between">
                <span className="text-muted-foreground font-semibold">الاسم الكامل:</span>
                <span className="font-bold text-foreground">{student.parent.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground font-semibold">البريد الإلكتروني:</span>
                <span className="font-bold text-foreground">{student.parent.email}</span>
              </div>
              {student.parent.phone && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground font-semibold">رقم الجوال:</span>
                  <span className="font-bold text-foreground">{student.parent.phone}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-5 border border-border bg-accent/10 rounded-xl space-y-3">
            ...
            <div className="text-xs space-y-2">
              <div className="flex justify-between">
                <span className="text-muted-foreground font-semibold">تاريخ التسجيل بالمنصة:</span>
                <span className="font-bold text-foreground">{new Date(student.createdAt).toLocaleDateString('ar-EG')}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground font-semibold">حالة الحساب:</span>
                <span className={cn("font-bold text-xs", student.isActive ? "text-emerald-600" : "text-rose-500")}>
                ...
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-muted-foreground">
                      المعلم: <strong className="text-foreground">{booking.teacherService.teacher.user.name}</strong>
                    </p>
                  ...
                  <div className="text-right flex flex-col justify-between items-end gap-2">
                    <span className="font-extrabold text-primary block">
                      {booking.isTrial ? 'تجريبية مجانية' : formatPrice(Number(booking.price))}
                    </span>
