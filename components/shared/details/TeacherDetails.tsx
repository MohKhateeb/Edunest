...
import { 
  User, CheckCircle2, Star, FileText, Award, DollarSign, 
  MapPin, GraduationCap, Mail, Phone, ExternalLink, BookOpen, Clock 
} from 'lucide-react';
...
export default function TeacherDetails({ teacher, activeTab, setActiveTab }: TeacherDetailsProps) {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="p-4 border border-border bg-accent/10 rounded-xl text-center">
          ...
          <span className="text-[10px] text-muted-foreground block font-bold">الحصص المنفذة</span>
          <span className="text-sm font-extrabold text-foreground">{teacher.totalSessions} حصة</span>
        </div>
        <div className="p-4 border border-border bg-accent/10 rounded-xl text-center">
          <Award className="h-5 w-5 text-primary mx-auto mb-1" />
          <span className="text-[10px] text-muted-foreground block font-bold">الخبرة</span>
          <span className="text-sm font-extrabold text-foreground">{teacher.yearsOfExperience} سنوات</span>
        </div>
        <div className="p-4 border border-border bg-accent/10 rounded-xl text-center">
          <DollarSign className="h-5 w-5 text-primary mx-auto mb-1" />
          <span className="text-[10px] text-muted-foreground block font-bold">السعر الافتراضي</span>
        ...
        <div className="space-y-4 text-xs">
          ...
          {teacher.bio && (
            <div className="p-4 border border-border bg-card rounded-xl">
              <span className="font-bold text-primary block mb-2 text-[11px]">النبذة الشخصية:</span>
              <p className="leading-relaxed text-foreground/80 italic">{teacher.bio}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 border border-border bg-card rounded-xl space-y-2">
              <span className="font-bold text-primary block mb-1 text-[11px]">الموقع والمستوى الدراسي:</span>
              <div className="flex items-center gap-1.5 text-foreground/85">
                <MapPin className="h-4 w-4 text-muted-foreground" />
                <span>المدينة والمنطقة: <strong>{teacher.city || 'غير محدد'}</strong> {teacher.area && `(${teacher.area})`}</span>
              </div>
              <div className="flex items-center gap-1.5 text-foreground/85">
                <GraduationCap className="h-4 w-4 text-muted-foreground" />
                <span>الصفوف التي يدرسها: <strong>{teacher.gradeLevels?.map((g: number) => `الصف ${g}`).join(', ') || 'غير محدد'}</strong></span>
              </div>
            </div>

            <div className="p-4 border border-border bg-card rounded-xl space-y-2">
              <span className="font-bold text-primary block mb-1 text-[11px]">بيانات الاتصال والتعليم:</span>
              <div className="flex items-center gap-1.5 text-foreground/85">
                <BookOpen className="h-4 w-4 text-muted-foreground" />
                <span>المؤهل العلمي: <strong>{teacher.education || 'غير محدد'}</strong></span>
              </div>
              <div className="flex items-center gap-1.5 text-foreground/85">
                <Mail className="h-4 w-4 text-muted-foreground" />
                <span>البريد الإلكتروني: <strong>{teacher.user.email}</strong></span>
              </div>
              {teacher.user.phone && (
                <div className="flex items-center gap-1.5 text-foreground/85">
                  <Phone className="h-4 w-4 text-muted-foreground" />
                  <span>رقم الهاتف: <strong>{teacher.user.phone}</strong></span>
                </div>
              )}
            </div>
          </div>
          ...
          {Object.prototype.hasOwnProperty.call(teacher, 'verification') && (
            <div className="p-4 border border-border bg-card rounded-xl space-y-3">
              <span className="font-bold text-primary block border-b border-border pb-1.5 text-[11px]">مستندات التحقق المرفوعة (للمدراء فقط):</span>
              {teacher.verification ? (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="p-3 border border-border bg-accent/15 rounded-xl space-y-1.5 flex flex-col justify-between">
                    <span className="font-bold text-[10px] text-muted-foreground block">البطاقة الشخصية (هوية)</span>
                    {teacher.verification.nationalIdUrl ? (
                      ...
                      <span className="text-[10px] text-rose-500 font-semibold block mt-1">غير مرفوعة</span>
                    )}
                  </div>

                  <div className="p-3 border border-border bg-accent/15 rounded-xl space-y-1.5 flex flex-col justify-between">
                    <span className="font-bold text-[10px] text-muted-foreground block">الشهادة الجامعية</span>
                  ...
                  <div className="p-3 border border-border bg-accent/15 rounded-xl space-y-1.5 flex flex-col justify-between">
                    <span className="font-bold text-[10px] text-muted-foreground block">الفيديو التعريفي</span>
                    {teacher.verification.videoInterviewUrl ? (
                      ...
                      <span className="text-[10px] text-muted-foreground block mt-1 italic">غير مرفوع</span>
                    )}
                  </div>
                </div>
              ) : (
                <p className="text-xs text-muted-foreground italic">لا توجد مستندات مرفوعة لهذا المعلم حالياً (تم توثيقه مباشرة من الإدارة أو لم يقم برفع وثائقه بعد).</p>
              )}
            </div>
          )}
        </div>
          ...
          {teacher.services.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {teacher.services.map((srv: any) => (
                <div key={srv.id} className="p-4 border border-border bg-card rounded-xl text-xs space-y-2 flex flex-col justify-between">
                  <div>
                    ...
                    {srv.customDescription && (
                      <p className="text-[11px] text-muted-foreground leading-relaxed mt-1">{srv.customDescription}</p>
                    )}
                  </div>
                  <div className="flex justify-between items-center border-t border-border/50 pt-2 mt-2">
                    ...
                    <span className="font-extrabold text-primary text-sm">{formatPrice(Number(srv.price))}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-3 max-h-[40vh] overflow-y-auto pe-1">
          {teacher.reviews.length === 0 ? (
            ...
            teacher.reviews.map((rev: any) => (
              <div key={rev.id} className="p-4 border border-border bg-card rounded-xl text-xs space-y-2">
                <div className="flex justify-between items-center">
                  <span className="font-bold text-foreground">{rev.booking.student.name || 'ولي أمر'}</span>
                ...
                <span className="text-[10px] text-muted-foreground/50 block mt-1">
                  تاريخ التقييم: {new Date(rev.createdAt).toLocaleDateString('ar-EG')}
                </span>
              </div>
            ))
          )}
