...
import { AlertCircle, Calendar, Clock, Moon, Sun } from "lucide-react";
		...
		windows.forEach((w) => {
			...
			while (current.getTime() + duration * 60_000 <= endLimit.getTime()) {
				...
				const overlaps = bookingsOnDate.some((b) => {
					...
					return (
						Math.max(slotStart.getTime(), bStart) <
						Math.min(slotEnd.getTime(), bEnd)
					);
				});
								...
								<div className="space-y-2">
									...
									<div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
										{morningSlots.map((slot, idx) => {
											const isSelected =
												selectedSlotTime === slot.date.getTime();
											return (
												<button
													key={`morning-${idx}`}
													type="button"
