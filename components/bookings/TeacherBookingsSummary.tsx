...
import { AlertTriangle, Clock, CalendarCheck, TrendingUp } from 'lucide-react';
...
export function TeacherBookingsSummary({ bookings }: TeacherBookingsSummaryProps) {
  const stats = useMemo(() => {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <div className="bg-rose-50/50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900 rounded-2xl p-4 flex items-start gap-4 transition-all hover:shadow-md">
        ...
        <div>
          <p className="text-xs font-bold text-muted-foreground mb-1">تقارير متأخرة</p>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-black text-rose-600 dark:text-rose-400">{stats.ghostCount}</span>
            <span className="text-[10px] text-rose-500 font-semibold">جلسة معلقة</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-amber-50/50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900 rounded-2xl p-4 flex items-start gap-4 transition-all hover:shadow-md">
        ...
        <div>
          <p className="text-xs font-bold text-muted-foreground mb-1">بانتظار موافقتك</p>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-black text-amber-600 dark:text-amber-400">{stats.pendingCount}</span>
            <span className="text-[10px] text-amber-500 font-semibold">طلب حجز</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900 rounded-2xl p-4 flex items-start gap-4 transition-all hover:shadow-md">
        ...
        <div>
          <p className="text-xs font-bold text-muted-foreground mb-1">حصص اليوم</p>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-black text-emerald-600 dark:text-emerald-400">{stats.todayCount}</span>
            <span className="text-[10px] text-emerald-500 font-semibold">مجدولة</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900 rounded-2xl p-4 flex items-start gap-4 transition-all hover:shadow-md">
        ...
        <div>
          <p className="text-xs font-bold text-muted-foreground mb-1">إنجاز الشهر</p>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-black text-blue-600 dark:text-blue-400">{stats.completedThisMonth}</span>
            <span className="text-[10px] text-blue-500 font-semibold">مكتملة</span>
          </div>
        </div>
      </div>
    </div>
