...
export function TeacherCalendar({
  ...
  const calendarCells = useMemo(() => {
    ...
    for (let i = firstDayIndex - 1; i >= 0; i--) {
      ...
      cells.push({ date: d, isCurrentMonth: false, dateStr: getLocalDateString(d) });
    }

    // Current month days
    for (let day = 1; day <= daysInMonth; day++) {
      const d = new Date(year, month, day, 12, 0, 0);
      cells.push({ date: d, isCurrentMonth: true, dateStr: getLocalDateString(d) });
    }
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-border/80 rounded-3xl p-5 shadow-sm">
      ...
      <div className="flex items-center justify-between mb-4 bg-accent/40 rounded-xl p-1.5 border border-border">
        <button onClick={handleNextMonth} className="p-1.5 hover:bg-background rounded-lg text-muted-foreground transition-colors">
          <ChevronRight className="h-4 w-4" />
        </button>
        <span className="text-xs font-black">
          {MONTHS_AR[month]} {year}
        </span>
        <button onClick={handlePrevMonth} className="p-1.5 hover:bg-background rounded-lg text-muted-foreground transition-colors">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isCurrentMonth ? "hover:bg-accent/50 cursor-pointer" : "text-muted-foreground/30",
                isSelected && "bg-primary text-primary-foreground font-black shadow-md hover:bg-primary",
                !isSelected && isToday && "border border-primary text-primary font-bold bg-primary/5"
              )}
            >
