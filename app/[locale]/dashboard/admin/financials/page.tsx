						...
						<div className="overflow-x-auto">
							<table className="w-full text-start">
								...
								<tbody className="divide-y divide-border">
									{revenueDetails.map(transaction => (
										<tr key={transaction.id} className="hover:bg-muted/30 transition-colors">
											<td className="p-4 font-mono text-sm">{transaction.date.toLocaleDateString("ar-SA")}</td>
											<td className="p-4">
												<span className={`text-xs px-2 py-1 rounded-md font-bold ${transaction.type === "COMMISSION" ? "bg-blue-100 text-blue-700" : "bg-red-100 text-red-700"}`}>
									...
									{resolvedEscrows.map((escrow) => (
										<div key={escrow.id} className="bg-muted/30 border border-border rounded-2xl p-4 flex justify-between items-center">
											...
											<div className="text-xs font-bold px-3 py-1.5 rounded-lg bg-background border border-border">
												{escrow.status === "REFUNDED_TO_PARENT" && t('key_1783109434439_6bcx')}
												{escrow.status === "PAID_TO_TEACHER" && t('key_1783109434448_7j0k')}
												{escrow.status === "PLATFORM_PROFIT" && t('key_1783109434455_6inp')}
											</div>
										</div>
									))}
								</div>
