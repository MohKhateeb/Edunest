...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. التحقق من حجم الملف (الحد الأقصى 50MB لدعم رفع الفيديوهات التعريفية)
    const MAX_FILE_SIZE = 50 * 1024 * 1024;
    if (file.size > MAX_FILE_SIZE) {
      return NextResponse.json({ error: 'حجم الملف يتجاوز الحد الأقصى المسموح به (50MB)' }, { status: 400 });
    }

    // 3. التحقق من نوع الملف (الصور، ملفات PDF، ومقاطع الفيديو)
    const ALLOWED_TYPES = [
      'image/jpeg', 'image/png', 'image/webp', 'application/pdf',
      'video/mp4', 'video/webm', 'video/ogg', 'video/quicktime', 'video/3gpp'
    ];
    if (!ALLOWED_TYPES.includes(file.type)) {
      return NextResponse.json({ error: 'نوع الملف غير مسموح به. مسموح بالصور، ملفات PDF، ومقاطع الفيديو الشائعة' }, { status: 400 });
    }
    ...
    if (!uploadSuccess) {
      ...
      fileUrl = `/uploads/${bucket}/${session.user.id}/${localFileName}`;
    }
