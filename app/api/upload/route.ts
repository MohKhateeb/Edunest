...
function validateUploadRequest(
	...
	if (!ALLOWED_TYPES.includes(file.type)) {
		return NextResponse.json(
			{
				error:
					"نوع الملف غير مسموح به. مسموح بالصور، ملفات PDF، ومقاطع الفيديو الشائعة",
			},
			{ status: 400 },
		);
	}
	...
	try {
		...
		if (isRealSupabase) {
			try {
				...
				console.warn(
					"Supabase upload exception, falling back to local storage:",
					sbErr,
				);
			}
		}
		...
		const fallbackResult = await uploadToSupabaseFallback(
			fileUrl,
			uploadSuccess,
			session,
			bucket,
