...
import { CreditCard, History, RefreshCcw, HandCoins, AlertCircle } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">تابع جميع مدفوعاتك، واطلع على سجلات الاسترداد والنزاعات.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)] border border-gray-100/50 dark:border-gray-700/50 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className="text-sm font-bold text-gray-500 dark:text-gray-400">إجمالي المدفوعات</p>
          </div>
        ...
        <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-3xl p-6 text-white shadow-[0_8px_30px_rgb(16,185,129,0.2)] relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <p className="text-sm font-bold text-emerald-50">المبالغ المستردة</p>
            </div>
      ...
      <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-2xl p-5 shadow-sm border border-gray-100/50 dark:border-gray-700/50 flex flex-col sm:flex-row gap-4 relative z-20">
        <form className="flex-1 flex flex-col sm:flex-row gap-5 w-full">
          <div className="flex-1">
            <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wide">التاريخ</label>
            <input 
              type="date" 
              name="date"
          ...
          <div className="flex-1">
            <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wide">المعلم</label>
            <select 
              name="teacher"
          ...
          <div className="flex items-end gap-3">
            <button type="submit" className="w-full sm:w-auto bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-8 py-2.5 rounded-xl font-bold hover:bg-gray-800 dark:hover:bg-gray-100 transition-colors shadow-md">
              تصفية
            </button>
            {(dateFilter || teacherFilter) && (
              <Link href="/dashboard/parent/financials" className="text-red-500 text-sm font-bold hover:underline self-center bg-red-50 dark:bg-red-900/20 px-3 py-1.5 rounded-lg border border-red-100 dark:border-red-900/50 mb-1">
                إلغاء الفلتر
              </Link>
            )}
          </div>
          ...
          <table className="w-full text-right">
            <thead>
              <tr className="bg-gray-50/50 dark:bg-gray-900/30 border-b border-gray-100/50 dark:border-gray-700/50">
                ...
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">المعلم / الخدمة</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">الطالب</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">المبلغ</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">حالة الدفع</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">النزاع</th>
              </tr>
            </thead>
                  ...
                  <tr key={booking.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="font-bold text-gray-900 dark:text-white flex items-center gap-1.5">
                        <span className="w-5 h-5 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 text-[10px]">م</span>
                        {booking.teacherService.teacher.user.name}
                      </div>
                      <div className="text-xs text-gray-500 mt-1">{booking.teacherService.serviceType.name}</div>
                    </td>
                    <td className="px-6 py-4 text-sm font-medium text-gray-700 dark:text-gray-200">
                      <div className="flex items-center gap-1.5">
                        <span className="w-4 h-4 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center text-[8px]">ط</span>
                        {booking.student.name}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="font-extrabold text-gray-900 dark:text-white">{Number(booking.price).toFixed(2)}</span>
                      ...
                      {booking.paymentStatus === 'PAID' ? (
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          {booking.parentRefund && (
                            <span className={`text-[10px] font-bold ${booking.parentRefund.isPaid ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>
                              {booking.parentRefund.isPaid ? '✓ تم التحويل البنكي' : '⏳ قيد التحويل للإدارة'}
                            </span>
                          )}
                        </div>
                      ) : (
                    ...
                    <td className="px-6 py-4">
                      {booking.dispute ? (
                        <div className="flex flex-col gap-2 items-start">
                          ...
                          <Link href={`/dashboard/disputes/${booking.dispute.id}`} className="text-xs font-bold text-blue-600 hover:text-white bg-blue-50 hover:bg-blue-600 px-3 py-1.5 rounded-lg transition-all border border-blue-100 dark:border-blue-900/50">
                            متابعة النزاع
                          </Link>
                        </div>
                      ) : (
