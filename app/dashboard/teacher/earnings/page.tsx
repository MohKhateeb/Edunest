...
import { Wallet, Landmark, Clock, History, AlertCircle, FileText, CheckCircle2, ArrowLeft } from 'lucide-react';
...
export default async function TeacherEarningsPage({
  ...
  const disputedBookings = rawBookings.filter(b => b.dispute).sort((a, b) => {
    if (a.dispute!.status === 'OPEN' && b.dispute!.status !== 'OPEN') return -1;
    if (b.dispute!.status === 'OPEN' && a.dispute!.status !== 'OPEN') return 1;
    return (b.completedAt?.getTime() || 0) - (a.completedAt?.getTime() || 0);
  });

  const normalBookings = rawBookings.filter(b => !b.dispute).sort((a, b) => {
    return (b.completedAt?.getTime() || 0) - (a.completedAt?.getTime() || 0);
  });
  ...
  const tabs = [
    ...
    { id: 'disputes', label: 'النزاعات', icon: AlertCircle, badge: disputedBookings.filter(b => b.dispute!.status === 'OPEN').length }
  ];
      ...
      <div className="flex flex-col lg:flex-row gap-6 justify-between items-start">
        <div className="lg:w-1/2">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium leading-relaxed">
            محفظتك الذكية للتحكم بأموالك. راقب الجلسات المكتملة، وتتبع مستحقاتك بكل بساطة وبطريقة خالية من التشتت!
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in duration-300">
          ...
          <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)] border border-gray-100/50 dark:border-gray-700/50 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <p className="text-sm font-bold text-gray-500 dark:text-gray-400">إجمالي الأرباح المستلمة</p>
            </div>
          ...
          <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-3xl p-6 text-white shadow-[0_8px_30px_rgb(16,185,129,0.2)] relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-4">
                ...
                <p className="text-sm font-bold text-emerald-50">الرصيد المتاح للتسوية</p>
              </div>
              <h3 className="text-4xl font-extrabold flex items-baseline gap-1">
                {availableToPayout.toFixed(2)}
                <span className="text-lg font-medium text-emerald-100/80">شيكل</span>
              </h3>
            </div>
          </div>
          ...
          <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)] border border-amber-200 dark:border-amber-900/30 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <p className="text-sm font-bold text-gray-500 dark:text-gray-400">رصيد معلق أو قيد التحويل</p>
            </div>
            <h3 className="text-4xl font-extrabold text-gray-900 dark:text-white flex items-baseline gap-1">
        ...
        <div className="animate-in fade-in duration-300 space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
            {normalBookings.length === 0 ? (
              <div className="col-span-full py-12 text-center text-gray-500 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700">لا توجد جلسات مكتملة حتى الآن.</div>
            ) : (
              normalBookings.map((booking) => {
                ...
                return (
                  <div key={booking.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all flex flex-col justify-between gap-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-bold text-sm text-gray-900 dark:text-white line-clamp-1">{booking.teacherService.serviceType.name}</h3>
                        <p className="text-xs text-gray-500 mt-1.5 flex items-center gap-1.5">
                          <span className="w-4 h-4 bg-blue-50 text-blue-600 dark:bg-blue-900/30 rounded-full flex items-center justify-center text-[9px] font-bold">ط</span>
                          {booking.student.name}
                        </p>
                      </div>
                      <div className="text-left shrink-0">
                        <span className="font-black text-xl text-gray-900 dark:text-white">{net.toFixed(2)}</span>
                        <span className="text-[10px] text-gray-500 mr-1">شيكل</span>
                        {booking.isTrial && <div className="text-[9px] bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded mt-0.5 font-medium text-center">تجريبية</div>}
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center border-t border-gray-100 dark:border-gray-700 pt-3">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <Link href={`/dashboard/teacher/bookings/${booking.id}`} className="text-[11px] font-bold text-blue-600 hover:text-blue-700 hover:underline">
                          عرض الجلسة &larr;
                        </Link>
                      </div>
                      <div>
                        {booking.payoutId ? (
                           <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300 text-[10px] font-bold">
                             <CheckCircle2 className="w-3 h-3 text-emerald-500" /> تم تسويتها
                           </span>
                        ) : isUnder24h ? (
        ...
        <div className="animate-in fade-in duration-300 space-y-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50 dark:bg-gray-800/50 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 gap-4">
            ...
            <form className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <button type="submit" className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-4 py-2 rounded-xl text-sm font-medium hover:bg-gray-800 dark:hover:bg-gray-100 transition-colors shrink-0">
                تصفية
              </button>
            </form>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {payouts.length === 0 ? (
               <div className="col-span-full py-12 text-center text-gray-500 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700">لا توجد تسويات مالية سابقة.</div>
            ) : (
              (resolvedParams.status ? payouts.filter(p => resolvedParams.status === 'PAID' ? p.isPaid : !p.isPaid) : payouts).map((payout) => (
                <div key={payout.id} className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all flex flex-col gap-4">
                  <div className="flex justify-between items-center">
                    ...
                    {payout.isPaid ? (
                      ...
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">
                        <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span> قيد التحويل البنكي
                      </span>
                    )}
                  </div>
                  <div>
                    <span className="font-extrabold text-3xl text-gray-900 dark:text-white">{Number(payout.netAmount).toFixed(2)}</span>
                    <span className="text-sm text-gray-500 mr-1 font-bold">شيكل</span>
                  </div>
                  <div className="border-t border-gray-100 dark:border-gray-700 pt-3 text-xs text-gray-500 flex justify-between items-center">
                    <span className="font-mono">{payout.createdAt.toLocaleDateString('ar-SA')}</span>
                    <span className="flex gap-1.5 items-center bg-gray-50 dark:bg-gray-900/50 px-2 py-1 rounded-md font-mono">
                      {payout.periodStart.toLocaleDateString('ar-SA')} <ArrowLeft className="w-3 h-3 text-gray-300" /> {payout.periodEnd.toLocaleDateString('ar-SA')}
                    </span>
                  </div>
                </div>
              ))
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {disputedBookings.length === 0 ? (
              <div className="col-span-full py-12 text-center text-gray-500 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 flex flex-col items-center gap-2">
                ...
                <span className="font-bold">لا توجد أي نزاعات مالية. أداؤك ممتاز!</span>
              </div>
            ) : (
              disputedBookings.map((booking) => {
                ...
                return (
                  <div key={booking.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-red-100 dark:border-red-900/30 shadow-sm hover:shadow-md transition-all flex flex-col justify-between gap-3 relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-start pr-3">
                      <div>
                        <h3 className="font-bold text-sm text-gray-900 dark:text-white line-clamp-1">{booking.teacherService.serviceType.name}</h3>
                        <p className="text-xs text-gray-500 mt-1.5 flex items-center gap-1.5">
                          <span className="w-4 h-4 bg-red-50 text-red-600 dark:bg-red-900/30 rounded-full flex items-center justify-center text-[9px] font-bold">ط</span>
                          {booking.student.name}
                        </p>
                      </div>
                      <div className="text-left shrink-0">
                    ...
                    <div className="flex justify-between items-center border-t border-gray-100 dark:border-gray-700 pt-3 mt-1 pr-3">
                       <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                         booking.dispute!.status === 'OPEN' 
                           ...
                           : booking.dispute!.status === 'RESOLVED_IN_FAVOR_OF_TEACHER'
                         ...
                         {booking.dispute!.status === 'OPEN' ? (
                           <><span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span> قيد المراجعة</>
                         ) : booking.dispute!.status === 'RESOLVED_IN_FAVOR_OF_TEACHER' ? (
                       ...
                       <div className="flex items-center gap-2">
                         <Link href={`/dashboard/teacher/bookings/${booking.id}`} className="text-[10px] font-bold text-gray-500 hover:text-gray-900 dark:hover:text-white hover:underline transition-all">
                           تفاصيل الجلسة
                         </Link>
                         <Link href={`/dashboard/disputes/${booking.dispute!.id}`} className="text-[10px] font-bold text-white bg-gray-900 dark:bg-white dark:text-gray-900 hover:bg-blue-600 dark:hover:bg-blue-500 px-3 py-1.5 rounded-lg transition-all shadow-sm">
                           المحادثة
                         </Link>
                       </div>
                    </div>
