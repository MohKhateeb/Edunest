...
import { ArrowRight, Calendar, Clock, CreditCard, AlertCircle, FileText, User, CheckCircle2, Wallet, Landmark } from 'lucide-react';
...
export default async function TeacherBookingDetailsPage({
  ...
  const netProfit = booking.isTrial ? Number(booking.trialCostToPlatform) : (price - commissionAmount);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <Link href="/dashboard/teacher/earnings?tab=history" className="inline-flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-blue-600 transition-colors mb-2">
        ...
        <div>
          <span className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold border ${statusColors[booking.status]}`}>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="font-bold text-gray-900 dark:text-white text-lg">{booking.teacherService.serviceType.name}</p>
                {booking.isTrial && (
                  <span className="inline-block mt-2 text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded font-bold">جلسة تجريبية مخفضة</span>
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1 text-gray-600 dark:text-gray-300">
                  ...
                  {booking.startTime.toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' })} - {endTime.toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' })}
                </div>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-bold text-gray-900 dark:text-white text-lg">{booking.student.name}</p>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              {!booking.isTrial && (
                <div className="flex justify-between items-center text-sm">
                  <span className="text-gray-500">عمولة المنصة ({commissionRate}%)</span>
                  <span className="font-bold text-red-500">-{commissionAmount.toFixed(2)} ₪</span>
                </div>
              )}
              ...
              <div className="pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center">
                <span className="font-bold text-gray-900 dark:text-white">الصافي المستحق لك</span>
                <span className="font-black text-2xl text-emerald-600 dark:text-emerald-400">{netProfit.toFixed(2)} ₪</span>
              </div>
            </div>
            ...
            <div className="mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
               {booking.payoutId ? (
                 <div className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 p-3 rounded-xl flex flex-col gap-1 border border-emerald-100 dark:border-emerald-900/30">
                   ...
                   <Link href="/dashboard/teacher/earnings?tab=payouts" className="text-xs underline hover:text-emerald-800 transition-colors">
            ...
            <div className="bg-red-50 dark:bg-red-900/10 rounded-3xl p-6 border border-red-100 dark:border-red-900/30 shadow-sm relative overflow-hidden group">
              ...
              <div className="mb-4">
                 <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold ${
                   ...
                   {booking.dispute.status === 'OPEN' ? (
                     <><span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span> قيد المراجعة</>
                   ) : booking.dispute.status === 'RESOLVED_IN_FAVOR_OF_TEACHER' ? (
              ...
              <Link href={`/dashboard/disputes/${booking.dispute.id}`} className="flex justify-center items-center gap-2 w-full text-sm font-bold text-white bg-gray-900 dark:bg-white dark:text-gray-900 hover:bg-blue-600 dark:hover:bg-blue-500 px-4 py-3 rounded-xl transition-all shadow-sm">
