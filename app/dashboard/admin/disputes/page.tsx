    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">مراقبة ومعالجة جميع النزاعات بين أولياء الأمور والمعلمين.</p>
        </div>
      </div>
      ...
      <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-3xl p-5 shadow-sm border border-gray-100/50 dark:border-gray-700/50 flex flex-col sm:flex-row gap-4">
        <form className="flex-1 flex flex-col sm:flex-row gap-5 w-full items-end">
          <div className="flex-1 w-full">
            <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wide">حالة النزاع</label>
            <select 
              name="status"
            ...
            >
              ...
              <option value="RESOLVED_IN_FAVOR_OF_PARENT">مغلق (لصالح ولي الأمر)</option>
              <option value="RESOLVED_IN_FAVOR_OF_TEACHER">مغلق (لصالح المعلم)</option>
            </select>
          </div>
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            {statusFilter && (
              <Link href="/dashboard/admin/disputes" className="text-red-500 text-sm font-bold hover:underline self-center bg-red-50 dark:bg-red-900/20 px-3 py-1.5 rounded-lg border border-red-100 dark:border-red-900/50 mb-1">
                إلغاء
              </Link>
            )}
          </div>
          ...
          <table className="w-full text-right">
            <thead>
              <tr className="bg-gray-50/50 dark:bg-gray-900/30 border-b border-gray-100/50 dark:border-gray-700/50">
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">تاريخ الجلسة</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">الأطراف</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">سبب النزاع</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">الحالة</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">إجراء</th>
              </tr>
            </thead>
                  ...
                  <tr key={dispute.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="font-bold text-gray-900 dark:text-white flex items-center gap-1.5 mt-2">
                        <span className="w-5 h-5 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center text-amber-600 text-[10px]">أ</span>
                        {dispute.booking.parent.name}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <p className="text-xs text-gray-600 dark:text-gray-400 line-clamp-2 max-w-xs">{dispute.reason}</p>
                    </td>
                    <td className="px-6 py-4">
                      {dispute.status === 'OPEN' ? (
                    ...
                    <td className="px-6 py-4">
                      <Link href={`/dashboard/disputes/${dispute.id}`} className="text-blue-600 hover:text-white hover:bg-blue-600 border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 text-xs font-bold px-4 py-2 rounded-xl flex items-center justify-center gap-1 transition-all max-w-fit">
                        التفاصيل
