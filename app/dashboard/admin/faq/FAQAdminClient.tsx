...
export default function FAQAdminClient({ initialFaqs }: { initialFaqs: FAQ[] }) {
  ...
  const [activeTab, setActiveTab] = useState<FAQCategory>(FAQCategory.PARENT);
  ...
  const filteredFaqs = faqs.filter((f) => f.category === activeTab).sort((a, b) => a.order - b.order);
            ...
            {filteredFaqs.length > 0 ? (
              filteredFaqs.map((faq) => (
                <tr key={faq.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-gray-900">{faq.order}</td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="flex items-center gap-3 bg-gray-50 p-4 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-900">حالة السؤال</p>
                  <p className="text-xs text-gray-500">هل ترغب في عرض هذا السؤال للمستخدمين؟</p>
                </div>
              </div>
