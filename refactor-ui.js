...
function patchTimeFirstBookingForm() {
  const file = path.join(__dirname, 'components/shared/TimeFirstBookingForm.tsx');
  ...
  // Replace state
  const stateRegex = /\/\/ حالة الإرسال[\s\S]*?const \[success, setSuccess\] = useState\(false\);/;
  content = content.replace(stateRegex, `// حالة الإرسال\n  const { createdBooking, loading, errorMsg, setErrorMsg, success, submitBooking, closePaymentModal } = useBookingSubmission();`);

  // Replace submit
  const submitRegex = new RegExp("const handleBookingSubmit = async \\\\(e: React\\\\.FormEvent\\\\) => \\\\{[\\\\s\\\\S]*?// عرض اسم الوقت المختار");
  ...
  const noStudentsRegex = /if \(students\.length === 0\) {[\s\S]*?return \([\s\S]*?<div className="bg-card border border-border rounded-xl p-8 text-center max-w-md mx-auto space-y-4">[\s\S]*?<\/div>[\s\S]*?\);[\s\S]*?}/;
  content = content.replace(noStudentsRegex, `if (students.length === 0) {
    return <NoStudentsState />;
  }`);

  // Replace BookingSuccessState
  const successStateRegex = /if \(success\) {[\s\S]*?return \([\s\S]*?<div className="bg-card border border-border rounded-2xl p-8 shadow-sm max-w-2xl mx-auto">[\s\S]*?<\/div>[\s\S]*?\);[\s\S]*?}/;
  content = content.replace(successStateRegex, `if (success) {
    return <BookingSuccessState />;
  }`);

  // Replace PaymentModal onClose
  const paymentModalRegex = /<PaymentModal[\s\S]*?onClose={\(\) => {[\s\S]*?}}[\s\S]*?\/>/;
  content = content.replace(paymentModalRegex, `<PaymentModal
  ...
  // Replace TrialToggle
  const trialRegex = /\{\!hasUsedTrial && \([\s\S]*?<div className="flex items-center gap-2 p-3 bg-purple-50[\s\S]*?<\/div>[\s\S]*?\)\}/;
  content = content.replace(trialRegex, `{!hasUsedTrial && (
              <TrialToggle isTrial={bookingDetails.isTrial} onChange={(c) => handleBookingChange('isTrial', c)} />
            )}`);

  // Replace QuickQuestionFields
  const quickQRegex = /\{activeService\?\.serviceTypeName === 'شرح مسألة سريعة' && \([\s\S]*?<div className="bg-accent\/40 border border-border rounded-xl p-4 space-y-3 animate-fadeIn">[\s\S]*?<\/div>[\s\S]*?\)\}/;
  content = content.replace(quickQRegex, `{activeService?.serviceTypeName === 'شرح مسألة سريعة' && (
              <QuickQuestionFields
...
function patchNewBookingForm() {
  ...
  // Replace state
  const stateRegex = /const \[createdBooking, setCreatedBooking\] = useState[\s\S]*?const \[success, setSuccess\] = useState\(false\);/;
  content = content.replace(stateRegex, `const { createdBooking, loading, errorMsg, setErrorMsg, success, submitBooking, closePaymentModal } = useBookingSubmission();`);

  // Replace submit
  const submitRegex = /const handleBookingSubmit = async \(e: React\.FormEvent\) => {[\s\S]*?if \(students\.length === 0\)/;
  const newSubmit = `const handleBookingSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  // Replace BookingSuccessState
  const successStateRegex = /\{success \? \([\s\S]*?<div className="text-center py-8 space-y-3">[\s\S]*?<\/div>[\s\S]*?\) : \(/;
  content = content.replace(successStateRegex, `{success ? (\n          <BookingSuccessState />\n        ) : (`);
  ...
  // Replace TrialToggle
  const trialRegex = /\{formData\.selectedServiceId && \!hasUsedTrial && \([\s\S]*?<div className="flex items-center gap-2 p-3 bg-purple-50[\s\S]*?<\/div>[\s\S]*?\)\}/;
  content = content.replace(trialRegex, `{formData.selectedServiceId && !hasUsedTrial && (
              <TrialToggle isTrial={formData.isTrial} onChange={(c) => handleCheckboxChange('isTrial', c)} />
            )}`);

  // Replace QuickQuestionFields
  const quickQRegex = /\{activeService\?\.serviceType\.name === 'شرح مسألة سريعة' && \([\s\S]*?<div className="bg-accent\/40 border border-border rounded-xl p-4 space-y-3 animate-fadeIn">[\s\S]*?<\/div>[\s\S]*?\)\}/;
  content = content.replace(quickQRegex, `{activeService?.serviceType.name === 'شرح مسألة سريعة' && (
              <QuickQuestionFields
