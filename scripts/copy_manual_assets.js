...
files.forEach((file) => {
	if (file.startsWith("parent_dashboard_mockup") && file.endsWith(".png")) {
		fs.copyFileSync(
			...
			path.join(destDir, "parent_dashboard.png"),
		);
		console.log("Copied parent dashboard image");
	} else if (
		file.startsWith("teacher_dashboard_mockup") &&
		file.endsWith(".png")
	) {
		fs.copyFileSync(
			path.join(srcDir, file),
			path.join(destDir, "teacher_dashboard.png"),
		);
		console.log("Copied teacher dashboard image");
	} else if (
		file.startsWith("admin_dashboard_mockup") &&
		file.endsWith(".png")
	) {
		fs.copyFileSync(
			path.join(srcDir, file),
			path.join(destDir, "admin_dashboard.png"),
		);
