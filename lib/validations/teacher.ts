...
export const teacherProfileSchema = z.object({
  ...
  defaultHourlyRate: z.coerce.number().min(5, 'الحد الأدنى لسعر الساعة هو 5 شيكل'),
...
export const availabilityItemSchema = z.object({
  ...
  startTime: z.string().regex(/^([01][0-9]|2[0-3]):[0-5][0-9]$/, 'تنسيق الوقت غير صالح (HH:MM)'),
  endTime: z.string().regex(/^([01][0-9]|2[0-3]):[0-5][0-9]$/, 'تنسيق الوقت غير صالح (HH:MM)'),
});
...
export const teacherSlugSchema = z.object({
  slug: z.string()
    ...
    .regex(/^[a-zA-Z0-9\-]+$/, 'الرابط يجب أن يحتوي فقط على أحرف إنجليزية، أرقام، أو شرطات (-) بدون مسافات'),
});
