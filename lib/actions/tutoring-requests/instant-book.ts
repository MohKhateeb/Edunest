 ...
 */
export async function claimLiveRequest(requestId: string): Promise<ActionResponse<{ bookingId: string }>> {
  try {
    const { userId } = await requireAuth([UserType.TEACHER]);
    const teacher = await requireTeacherProfile(userId);

    if (!teacher.isVerified) {
      return { success: false, error: 'يجب توثيق حسابك أولاً لالتقاط الطلبات الفورية' };
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!teacher.gradeLevels.includes(request.student.grade)) {
        throw new Error('المرحلة الدراسية للطالب لا تقع ضمن المراحل التي تدرسها');
      }
      ...
      for (const b of activeBookings) {
        ...
        if (Math.max(requestedStartMs, otherStartMs) < Math.min(requestedEndMs, otherEndMs)) {
          throw new Error('لديك حجز آخر متداخل في هذا الوقت حالياً. يرجى إنهاء جلستك أولاً.');
        }
      }
    ...
    return { success: false, error: error.message || 'حدث خطأ أثناء التقاط الطلب' };
  }
}
