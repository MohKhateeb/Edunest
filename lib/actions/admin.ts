...
export async function confirmPayment(bookingId: string): Promise<ActionResponse> {
  try {
    ...
    console.error(err);
    const msg = err instanceof Error ? err.message : 'حدث خطأ أثناء تأكيد الدفع';
...
export async function toggleUserActive(
  ...
  try {
    const validated = toggleUserActiveSchema.safeParse({ targetUserId, isActive });
    if (!validated.success) {
      return { success: false, error: validated.error.issues[0].message };
    }
...
export async function updateHomepageLayout(layoutJson: string): Promise<ActionResponse> {
  try {
    const { userId: adminUserId } = await requireAuth([UserType.ADMIN]);
    ...
    try {
      ...
      return { success: false, error: 'تنسيق البيانات غير صالح (JSON غير صحيح)' };
    }
    ...
    console.error(err);
    return { success: false, error: 'حدث خطأ أثناء تحديث تخطيط الصفحة الرئيسية' };
  }
}
