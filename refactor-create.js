...
const oldFinancialBlockRegex =
	/\/\/ 4\. Fetch dynamic settings and calculate price\/duration\/commission[\s\S]*?(?=\/\/ 5\. Check Weekly Recurring Availability)/m;
...
const oldTeacherOverlap =
	/const otherStartMs = b\.startTime\.getTime\(\);\s*const otherEndMs = otherStartMs \+ b\.duration \* 60_000;\s*const hasOverlap = Math\.max\(requestedStartMs, otherStartMs\) < Math\.min\(requestedEndMs, otherEndMs\);/g;

content = content.replace(
	oldTeacherOverlap,
	"const hasOverlap = hasTimeOverlap(startTime, duration, b.startTime, b.duration);",
);
